...
async def test_project(dut):
    ...
    # 1. Verify reset state (after falling edge so outputs are settled)
    ...
    await FallingEdge(dut.clk)
    assert dut.uo_out.value == 0, f"Expected counter to hold 0, got {dut.uo_out.value}"
    ...
    # 6. Test bidirectional output enable (ui_in[2] = 1)
    dut._log.info("Test bidirectional output enable (uio_oe)")
    dut.ui_in.value = 0b100  # output_en = 1, count_en = 0
    await FallingEdge(dut.clk)
    assert dut.uio_oe.value == 0xFF, f"Expected uio_oe 0xFF, got {dut.uio_oe.value}"

    dut.ui_in.value = 0b000  # output_en = 0
    await FallingEdge(dut.clk)
    assert dut.uio_oe.value == 0x00, f"Expected uio_oe 0x00, got {dut.uio_oe.value}"
